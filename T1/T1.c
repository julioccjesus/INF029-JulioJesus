...
DiasMesesAnos q2(char datainicial[], char datafinal[])
{
    ...
    resultado.retorno = 1;
    // printf("Diferenca: %d anos /%d meses /%d dias cod %d\n", resultado.qtdAnos, resultado.qtdMeses, resultado.qtdDias, resultado.retorno);
...
int q4(char strTexto[252], char strBusca[50], int posicoes[30]) {
    ...
    for (int i = 0; i <= lenTexto - lenBusca; i++) {
        ...
        if (encontrou) {
            ...
            qtdOcorrencias++;
            i += lenBusca - 1;
            if (qtdOcorrencias * 2 + 1 >= 30) break;
        }
    }
